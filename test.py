...
def process_received_data():
    while not receive_queue.empty():
        ...
        # Update the GUI with the parsed results
        parsed_data_area.delete("1.0", tk.END)  # Clear previous content
        if parsed_data != "Unknown data format":
            parsed_data_area.insert(tk.END, parsed_data)
...
timestamp_checkbox.grid(row=4, column=1, padx=10, pady=5, sticky=W)
...
entry.grid(row=2, column=2, padx=10, pady=5, sticky=W)

# Send Button
send_button = tk.Button(window, text="Send", command=send_data)
send_button.grid(row=2, column=2, padx=10, pady=5, sticky=E)

# Text Area
text_area = tk.Text(window)
text_area.grid(row=3, column=0, columnspan=3, padx=10, pady=5, sticky=N+S+E+W)
...
clear_button.grid(row=4, column=2, padx=10, pady=5, sticky=E)

# Parsed Data Area
parsed_data_area = tk.Text(window)
parsed_data_area.grid(row=5, column=0, columnspan=3, padx=10, pady=5, sticky=N+S+E+W)
